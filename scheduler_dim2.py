...
class Scheduler(object):
    ...
    def initialize_seeds(self, seed):
        ...
        # self.worker_type_shuffler = random.Random()
        # self.worker_type_shuffler.seed(seed+5)

        # self.SLO_generator = random.Random()
        # self.SLO_generator.seed(seed+6)
    ...
    def get_target_job_status_update_path_and_status(self, job_id, operator):
        ...
        if operator == "gpu":
            ...
            if origin_status == JOB_STATUS_KEY.NO_SCHE:
                ...
                update_path = JOB_STATUS_UPDATE_PATH.DATASETSCHED_2_FAILED
                new_status = JOB_STATUS_KEY.FAILED
            elif origin_status == JOB_STATUS_KEY.DONE_GPU_SCHED:
                update_path = JOB_STATUS_UPDATE_PATH.GPUSCHED_2_FAILED
                new_status = JOB_STATUS_KEY.FAILED
    ...
    def sched_for_one_job(self, job_id):
        ...
        if (not need_failed_job) and job_id in (self.status_2_jobid[JOB_STATUS_KEY.NO_SCHE] + self.status_2_jobid[JOB_STATUS_KEY.DONE_GPU_SCHED]):
            dataset_name = self.jobid_2_datasettargetconfig[job_id]["dataset_name"]
        ...
        if (not need_failed_job) and job_id in self.status_2_jobid[JOB_STATUS_KEY.DONE_ALL_SCHED]:
            ...
            worker_identifier = self.jobid_2_gputarget[job_id]
            throughput_ratio = origin_info["throughput"][worker_identifier]
